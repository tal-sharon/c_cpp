...
/**
 * Checks if the given data is an end of a sentence - if it ends with a '.'
 * @param data_ptr a pointer to str, the given data to be checked
 * @return true: if data ends with '.', false: otherwise
 */
bool is_end (char *data_ptr);
...
bool
is_node_in_counter_list (const MarkovNode *first_node, MarkovNode *second_node)
{
  char *data_ptr = second_node->data;
  for (int i = 0; first_node->counter_list[i]; i++)
  {
    if (strcmp (first_node->counter_list[i]->markov_node->data, data_ptr) == 0)
    {
...
Node* get_node_from_database(MarkovChain *markov_chain, char *data_ptr)
{
  ...
  {
    if (strcmp (node_ptr->data->data, data_ptr) == 0) //found data:return node
    {
      return node_ptr;
    }
    node_ptr = node_ptr->next;
...
//  NextNodeCounter *next_node_counter = malloc (sizeof (NextNodeCounter));
  ...
  char *temp = malloc (sizeof (char)* (strlen (data_ptr) +1));
  if (temp == NULL)
  {
    return NULL;
  }
  memcpy (temp, data_ptr, sizeof (char)* (strlen (data_ptr) +1));
